...
QDataStream& operator<<(QDataStream& os,const start_rec& s){
    os<<s.ID<<s.EndTime<<s.MaterialInf<<s.StartUser<<s.EndCount<<s.probatch<<s.StartTime<<s.DevID<<s.remaind_count;
        return os;
}
...
//    in>>s.ID>>s.EndTime>>s.MaterialInf>>s.StartUser>>s.EndCount>>s.probatch>>s.StartTime>>s.DevID>>s.remaind_count;
//        return in;
//}
...
void socket::my_connection() {
    ...
    connect(clientConnection,&QTcpSocket::disconnected,this,&socket::delete_connect);
    qDebug()<<"connected";
}
