    ...
    ui(new Ui::MainWindow)
{
    ...
    connect(my_socket.data(),&socket::redraw,this,&MainWindow::redraw);
#ifdef MY_P
    qDebug()<<"MY_P";
    ...
    //tablestart = QSharedPointer<my_table>(new my_table("开工记录",ui->label_2,ui->widget_2,"生产令号","完成时间","物料信息","员工号","成品数","开工时间","设备","剩余数"));
...
void MainWindow::change_status() {//进入开工/完工模块
    ...
    if (my_socket->get_status()) {//开工
        table2.data()->setVisible(true);
        qDebug()<<"开工";
        table->unhidden();
        ui->verticalLayout_2->addWidget(table2.data());
    }
    else {//完工
        table3.data()->setVisible(true);
        qDebug()<<"完工";
        table->hidden();
...
void MainWindow::my_send4() {//生产记录码20010001180321000101,200903115125
    if (!is_work){return;}
    QString temp = "20016101180122000101,1060207142815,210106224812,1,0,55045,001073,1800,0,0,,666,PQC\n666Y\n,外协机加进料检,17121618739,,,";//"20016101180122000101,201230212915,201230212915,1,0,55045,001073,500,0,0,,999,,外协机加进料检,17121618739,,,";//"20016101180122000101,,201027154713,1,0,YG,55045,SB,001073,2900,0,60,2222,,444,外协机加进料检,17121618739,,016101,";//"20016101180122000102,200819121338,200515163814,7,0,50000,037013,300,0,11,,,,,245973,,,1,0,0,1,50000,001073,100,0,200815164731,0,300";
...
void my_table::remove() {
    ...
    if (this->num == 0) {//只有start才删数据库
        my_socket->send_message(52,QString(this->item(this->currentRow(),0)->text()) + "," +this->item(this->currentRow(),1)->text());
    }
